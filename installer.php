...
function removeinstaller($temp)
{
    ...
    // header("Location: ./"); - Moved to jquery
}
 ...
 * @return bool
 */
function unphar($src, $dest)
{
    return true;
}

function plruncommand($tmppath)
{
    //Require composer
...
if(isset($_POST["ajax"]) && !empty($_POST["ajax"])){
    ...
    elseif(isset($_POST["ajax"]) == 'init')
    {
        ...
        while (!file_exists($tmppath . 'vendor/autoload.php')) {
            ...
            if ( $maxtries >= '3' ) {
                throw new exception("Could not extract PHAR");
        ...
        <script>
            ...
            {$( "#btncompose" ).replaceWith( '<h2 class="instal1">Downloading. Please wait.</h2>' );
                return $.ajax({
                    ...
                    .done(function(res) {
                        console.log(res);
                        $( ".install" ).replaceWith( '<span id="btncleanup" class="btn btn-primary btn-lg" role="button">Step 3: Cleanup</span>' );
                    })
                    .fail(function(err) {
                        console.log('Error: ' + err.status);
                        $( ".install" ).replaceWith( '<h2 class="instal1">Error:' + err.status + '</h2>' );
